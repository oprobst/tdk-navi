...
void setup() {
  ...
  delay (5000);
  byte sendMe [] = { 0xb5, 0x62, 0x06, 0x24, 0x00, 0x00, 0x00, 0x00};
  calcChecksum(&sendMe[2], sizeof(sendMe) - 4);
    
  Serial.print("Request configuration: ");   
   for (byte i = 0, j = 0; i < sizeof(sendMe); i++) {
      Serial.print(sendMe[i]);     
      Serial.print(' ');     
   }
   Serial.println();
  sendUBX(&sendMe[0], sizeof(sendMe));
  int i = 0;
  while (i < 50) {     
      
       if (gpsSerial.available()) {
 i++;
         uint8_t incoming_char = gpsSerial.read();
         printHex(&incoming_char, 1);
         //Serial.print("=");
  ...
  while (!complete) {
    ...
    if (gpsSerial.available()) {
      ...
      if (in == '*' || currentByteCount > (MAX_MSG_SIZE - 4)) {
        calcChecksum(&buffer[currWriteBufferIdx][3],  currentByteCount);
        currentByteCount+=2;
...
void configureGPS() {
  ...
  byte settingsArray[] = { 0x05, 0xE8, 0x03, 0x80, 0x25, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00
                         };
...
void configureUblox(byte *settingsArrayPointer) {
  ...
  byte setNav[] = { 0xB5, 0x62, 0x06, 0x24, 0x00, 0x00, 0xFF, 0xFF,
                    *settingsArrayPointer, 0x03, 0x00, 0x00, 0x00, 0x00, 0x10, 0x27,
                    0x00, 0x00, 0x05, 0x00, 0xFA, 0x00, 0xFA, 0x00, 0x64, 0x00, 0x2C,
  ...
  byte setDataRate[] = { 0xB5, 0x62, 0x06, 0x08, 0x06, 0x00,
                         settingsArrayPointer[1], settingsArrayPointer[2], 0x01, 0x00, 0x01,
                         0x00, 0x00, 0x00
                       };
  ...
  byte setPortRate[] = { 0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00,
                         0x00, 0xD0, 0x08, 0x00, 0x00, settingsArrayPointer[3],
                         settingsArrayPointer[4], settingsArrayPointer[5], 0x00, 0x07, 0x00,
  ...
  gpsSetSuccess = 0;
    ...
    byte lowerPortRate[] = { 0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00,
                             0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x80, 0x25, 0x00, 0x00,
                             0x07, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA2, 0xB5
                           };
